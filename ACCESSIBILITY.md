...
- **No form association yet** — components are **not** `ElementInternals`-based form-associated elements, so they don't participate in native `<form>` submission/validation; `b-form` collects values programmatically. This is a feature gap (not an SR gap) tracked in `tasks/EPIC-001/STORY-023/TASK-035`.
