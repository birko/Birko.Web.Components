...
export class BDatePicker extends BaseComponent {
  ...
  private _renderCalendar(): string {
    ...
    const todayLabel = this._todayLabel();
    const clearLabel = this._clearLabel();
    ...
    return `
      ...
      <div class="dp-footer">
        <button data-nav="today">${todayLabel}</button>
        <button data-nav="clear">${clearLabel}</button>
      </div>
    `;
  }
  ...
  protected onUpdated() {
    ...
    this.listen(panel, 'click', (e: Event) => {
      ...
      switch (nav) {
        ...
        case 'today': {
          const now = new Date();
          this._selectDate(toISO(now.getFullYear(), now.getMonth(), now.getDate()));
          return;
        }
        case 'clear':
          this._selectDate('');
...
define('b-date-picker', BDatePicker);
